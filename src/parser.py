...
class Parser:
    ...
    def parse_parts(self, name, args):
    ...
    def parse_data_declaration(self):
        ...
        while self.current_token and self.current_token[0] == 'ID':
            ...
            if self.current_token and self.current_token[0] == 'ASIGNACION':
                self.advance()
                if not self.current_token or self.current_token[0] not in ExpresionValues:
                    self.error(f"Se esperaba un valor despues de ← para la variable {var}")
                value = self.parse_expresion()
    ...
    def parse_reasignacion(self):
        ...
        while self.current_token:
            ...
            if self.current_token[0] != 'ID':
                self.error("Se esperaba una lista de identificadores en reasignacion")
            var_name = self.current_token[1]
            self.advance()
            reasig_vars.append(var_name)
        ...
        if not self.current_token or self.current_token[0] != 'ASIGNACION':
            self.error("Se esperaba el operador = para reasignacion")
        self.advance()
        ...
        if len(reasig_vars) != len(values):
            self.error("La cantidad de variables es distinta a la cantidad de valores en reasignacion")
    ...
    def parse_instruccion(self):
        ...
        if not self.current_token:
            ...
            if self.current_token and self.current_token[0] == 'OPENP':
                ...
                if self.current_token and self.current_token[0] in ('ASIGNACION', 'DEF'):
                    if self.current_token[0] == 'ASIGNACION':
                        ...
                        if self.current_token and self.current_token[0] == 'OPENL':
                            self.advance()
                            return self.parse_parts(first_id, [arg['value'] for arg in args if arg['type'] == 'id'])
                        else:
                            expr = self.parse_expresion()
                            return {
                                'type': 'function',
            ...
            base_node = {'type': 'id', 'value': first_id}
            ...
            while self.current_token and self.current_token[0] == 'COMA':
                ...
                self.advance()
                next_base = {'type': 'id', 'value': next_id}
